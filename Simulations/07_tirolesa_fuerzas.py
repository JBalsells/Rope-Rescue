...
def main():
    ...
    def update(_=None):
        ...
        # Flechas de tensión en anclajes (dirección exacta de la cuerda,
        # longitud proporcional a la tensión)
        arrow_scale = float(np.clip(T_kN * 0.3, L * 0.05, L * 0.25))
        head_scale  = float(np.clip(T_kN * 0.8, 10, 30))
        lw_arrow    = float(np.clip(T_kN * 0.15, 1.5, 5.0))
        for anchor_x in [0, L]:
            vec_x = load_x - anchor_x   # hacia el punto de carga
            vec_y = load_y              # load_y - 0
            norm = np.sqrt(vec_x**2 + vec_y**2)
            dx = arrow_scale * vec_x / norm
            dy = arrow_scale * vec_y / norm
            ax_tiro.annotate(
                '', xy=(anchor_x + dx, dy),
                xytext=(anchor_x, 0),
                arrowprops=dict(arrowstyle='->', color=COLORS['info'],
                                lw=lw_arrow, mutation_scale=head_scale))

...
import math
import pygame
from config import PG_COLORS as C, G, NFPA_WORK_LOAD, ROPE_STATIC_MBS
from physics import sling_tension
import pg_utils

WIDTH, HEIGHT = 1400, 850
FPS = 60

# ── Limites de seguridad (MBS y NFPA: fuente unica config) ──────────
ROPE_MBS = ROPE_STATIC_MBS   # kN cuerda estatica 11 mm
...
# ── Colores por anclaje (fósforo distinguible sobre fondo terminal) ──
ANCHOR_COLORS = [
    (0, 255, 51),      # Anclaje 1 - Verde
    (255, 176, 0),     # Anclaje 2 - Ámbar
    (0, 217, 160),     # Anclaje 3 - Teal
    (170, 255, 68),    # Anclaje 4 - Lima
]
...
# sling_tension(force_kn, alpha_deg) = F / cos(alpha) vive en physics.py.
...
class MultiAnchorSim:
    ...
    def __init__(self):
        ...
        pygame.display.set_caption('Distribucion Multi-Anclaje')
    ...
    def _draw_dashed_line(self, surface, color, start, end, width, dash_len):
        """Linea discontinua (delegado a pg_utils.dashed_line)."""
        pg_utils.dashed_line(surface, color, start, end, width, dash_len)
...
@simulation(backend='pygame', order=8,
            title='Distribución multi-anclaje (animado)',
            description='Reparto auto-ecualizado y fallo de un punto.')
def main():
